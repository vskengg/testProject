...
public class WebServiceClient {
	...
	public String replicateContent(String path, String action) throws JSONException {
		...
		try {
			...
			log.error("Exception in replicateContent is : " + e.toString());
		} finally {
	 ...
	 * @throws JSONException
	 */
	public String deactivateLater(String path, String action, Date date) throws JSONException {
		DefaultHttpClient client = new DefaultHttpClient();
		BufferedReader rd = null;
		String statusCode = null;
		try {
			...
			log.debug("action is : " + action);
			HttpHost targetHost = new HttpHost(hostName, 4502, "http");
			String actionurl = "/c/wcm/servlets/wemreplication.activateDeactivateLater.json";
			HttpPost post = new HttpPost(actionurl);
			post.addHeader("AUTH_USER", userId);
			List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(1);
			nameValuePairs.add(new BasicNameValuePair("assetpath", path));
			nameValuePairs.add(new BasicNameValuePair("cmd", action));
			nameValuePairs.add(new BasicNameValuePair("absoluteTime", String.valueOf(date.getTime())));
			nameValuePairs.add(new BasicNameValuePair("stopworkflow", "no"));
			post.setEntity(new UrlEncodedFormEntity(nameValuePairs));
			HttpResponse response = client.execute(targetHost, post);
			rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
			StringBuffer content = new StringBuffer();
			...
			log.error("Exception in deactivateLater is : ", e);
		} finally {
			if (rd != null) {
				try {
					rd.close();
				} catch (IOException e) {
					log.error("IOException in deactivateLater is : ", e);
				}
			}
			client.getConnectionManager().shutdown();
		}
	 ...
	 * WebServiceClient client = new WebServiceClient(); String filePath =
	 * "/content/dam/en/us/td/docs/concepts/11.doc"; //
